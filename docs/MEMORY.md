...
- **L4 (Option B)**: Resizable loupe — right edge / bottom edge / SE-corner drag handles resize `#loupe-card-container`. Min 200px, max fills viewport (minus 60px toolbar). Double-click any handle snaps back to default size via `_loupeResetSize()` (rect-shape aware). Drawing coords already use `getBoundingClientRect()`, so no `loupeCoords()` update needed. Handles live in `#loupe-overlay` (NOT inside the container — container has `overflow:hidden`+`border-radius:50%` for circle cards which would clip them); positions tracked via `ResizeObserver` and repositioned on window resize. Faint gold tint (`rgba(196,164,90,0.18)`) makes them discoverable; brightens to `0.55` on hover. Hidden on coarse-pointer devices via `@media (hover:none) and (pointer:coarse)`. Size **persists** across loupe opens via `localStorage.loupeSizeScale_v1 = {sx, sy}` (scale factor relative to default, so it adapts to different card shapes). Double-click any handle clears the persisted scale. Key helpers: `_ensureLoupeResizeHandles()`, `_attachLoupeResizeHandles()`, `_positionLoupeResizeHandles()`, `_computeLoupeDefaultSize()`, `_loupeResetSize()`, `_wireLoupeResizeHandle()`, `_applyPersistedLoupeScale()`, `_load/_save/_clearLoupeScale()`.
