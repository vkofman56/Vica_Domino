...
- **L4 (Option B)**: Resizable loupe — right edge / bottom edge / SE-corner drag handles resize `#loupe-card-container`. Min 200px, max fills viewport (minus 60px toolbar). Double-click any handle snaps back to default size via `_loupeResetSize()` (rect-shape aware). Drawing coords already use `getBoundingClientRect()`, so no `loupeCoords()` update needed. Handles are hidden on coarse-pointer devices via `@media (hover:none) and (pointer:coarse)`. Key helpers: `_ensureLoupeResizeHandles()`, `_attachLoupeResizeHandles()`, `_computeLoupeDefaultSize()`, `_loupeResetSize()`, `_wireLoupeResizeHandle()`. Called from all 3 openLoupe paths (`openLoupe` ~1489, variation opener ~2546) so handles survive `innerHTML=''` clears.
