...
**Date**: July 17, 2026 — **PARAMETRIC LAYOUT ENGINE COMPLETE + deployed** (deploy tip `bd9c785` on `claude/review-project-docs-JOOeh` + both mirrors; work tip `367cde6`). A full week (July 14–17) building the box/slot/adjust/row/column layout system on top of the July-13 alignment work. Read the [[parametric-cards-feature]] auto-memory for the running detail; the arc:

- **July 14 — three user bugs + font work.** Measured all 22 picker fonts: 9 have PROPORTIONAL digits, `tnum` can't fix them → font picker badges `123`/`1≠0`, per-face runtime probe (`_fontDigitsProbe` — MUST `document.fonts.load` first or it measures the Arial fallback). Free-width align was inverted (pinned a letter edge, grew the other way) — fixed editor AND player. **Sync gap**: the six Studio stores (`cardMathParams_v1`, `cardMathFormula_v1`, `cardMathRel_v1`, `cardAnswerLinks_v1`, `cardNoteLinks_v1`, `gameNoteLegends_v1`) were in NEITHER card-key NOR `_localWinsKeys`, so a stale cloud snapshot rolled Par edits back on every login (symptom: a space-box reserve un-checking itself after Save). Added to `_localWinsKeys` — see [[sync-local-wins-gap]].
- **July 15 — the box model.** Each placed parameter shows a SOLID box (shortest value) + DOTTED box (longest = the reserved slot). The slot's position is STORED (`data-slot-left/-w`), not derived from the letter — toggling moves the LETTER inside the fixed dotted box. Boxes = digit-count × the digit UNIT (average digit advance, or the WIDEST digit when **equidistant digits** is ticked → boxes become exact). Answer parameter (C) sizes from the FORMULA. Font advice is STACK-TRIGGERED (`_pmStackedParams`, compares BASELINES not bboxes). "equidistant digits" checkbox; reserve is a FLOOR with a `N digit(s) spill / extra space` readout.
- **July 16 — values render + ROW groups + packing.** VALUES render into the slot (`_pmPlaceValueInSlot` editor / `_paramSlotPlace` player, equidistant = per-char tspans, verified numerically identical). ADJUST followers rebase on the normal box. Per-group ▷ + card-level ▷ (width combinations). Standing `adjusted #N ✓` chips, numbered + movable, pulsing offer. ROW groups: 2+ params side-by-side pack as one line (the row extent HOLDS STILL, the line moves inside — `_pmRowPack`/`_rowPackFace`).
- **July 17 — COLUMN groups + help + a bug.** COLUMN groups: 2+ params one-under-another + a static ("+"); boxes = the NUMBERS only (solid=norm union, dotted=slot union); the static keeps ONE clearance to the column's NEAREST edge and follows it (union rule — the per-member-gap first cut overshot; user's "for 2 2-digit numbers + is too far"); green tether static→box (`_pmColPack`/`_colPackFace`). Card Editor **?** reorganized into COLLAPSIBLE FOLDERS (Card & view · Insert · Select & arrange · **Set parameters** · **Present parameters**). Spill readout number-first + right-aligned. Fixed: the identical-cards warning (position:fixed on body) lingered over the library after leaving Card Maker — closed at `returnToLibrary()`.

**Still open (user-deferred, in priority order):** (1) multi-ROW with a shared static — the static follows whichever member is WIDEST per instance set; (2) rows inside columns (two-axis multi-line problems); (3) word/sentence parameters reusing the equidistant machinery ("equidistant letters").

**Verification discipline this week:** every commit ran the JavaScriptCore inline-JS syntax gate (`osascript -l JavaScript` — node isn't installed) and was driven numerically in the live preview (:8012) before commit; editor↔player agreement is checked to the decimal on the same synthetic face because that pair has diverged twice. Deploy = merge work→deploy in a TEMP worktree (never switch the shared tree), push to all 3 branches; `git fetch` stays blocked under the classifier but a stale push just rejects, so fast-forward acceptance is the proof.

---

**Date**: July 13, 2026 — **ALIGNMENT FINALE: slot recapture per show, ADJUST-group tether, SVG toggle icons** (commits `00fd261` + the July 13 tip; see git log). The July 11–13 alignment round in full: (a) the alignment reference = the RESERVED SLOT (advance-measured n-digit run) — a full-width value renders at ONE position under every alignment; (b) the slot is **recomputed on EVERY 123 show** from the stored template identity — cards saved by older builds carried stale baked refs (`data-ref-*` in svgContent) that froze a too-narrow slot, making full-width values MOVE under the toggle, opposite the arrows (July 13 user report; verified fixed against poisoned refs); (c) **ADJUST groups** (opt-in "adjust" chip: one param + statics): statics follow the value's nearest edge, DELTA-model shifts (user drags survive any toggle order), stripped from every save; **dashed-green tether + member underline** drawn live by the param-boxes rAF loop shows the group in both template and instance views (ephemeral — stripped from saves and play faces); (d) free-width params: L/C/R = growth direction, honored in play via canvas advance measurement; (e) align toggle uses drawn SVG icons (geometrically centered — the old font glyphs sat low; also the freshness marker) and updates selection rings + followers on toggle; (f) homogeneous GROUP SCALING (2+ selected: sliders scale positions around the group center). **OPEN QUESTION for the next chat (July 13, user + assistant agreed to defer):** the align toggle on a FREE-WIDTH parameter (no space-box) confuses — with no slot, the toggle pins an EDGE of the template letter (right = value's right edge sits at the letter's right edge), so a value WIDER than the letter grows LEFTWARD when pressing the right arrow: the pinned edge matches the arrow but the MOTION opposes it. With a reserve checked, motion and arrow agree (that path is verified correct). Candidate designs to discuss: hide/disable the toggle when no reserve is set; different icons/tooltip semantics ("growth direction"); or auto-reserve from the range's max digits. LESSON for verification loops: the deploy banner bumps ONLY on commit — uncommitted fixes are invisible to it; commit checkpoints eagerly while the user tests.

---

**Prior — July 11, 2026** — **`docs/GAME_INVARIANTS.md` created + parameter ALIGNMENT honored in play + homogeneous GROUP SCALING**. (1) New standing document **`docs/GAME_INVARIANTS.md`** (user request): the checklist of everything auto-applied to ALL games and required of future ones — secrecy, author-chrome stripping, parametric-play rules, board invariants, card-set inheritance, edit guards, play entry points. Extend it in the same commit whenever an always-on behavior ships. (2) **Per-parameter L/C/R alignment survives into play** (Phase 1 of the alignment design): `_pfBakeFindGame` now ships RAW value + reserved width per sym (`{v,w}`; hidden stays `''`), and `_paramApplyFace` pads figure-spaces per each face element's `data-align` (left→pad right, right→pad left, center→split) — verified all three on the board; legacy pre-padded bakes still play until the next ▶ re-bake. Space-boxes + center-align = a problem line that stays visually centered for every instance. Phase 2 (card-level block anchoring for free-width lines) is designed but NOT built — only if space-boxes prove insufficient. (3) **Homogeneous group scaling in the loupe**: with 2+ elements selected, the size/×-overscale sliders scale POSITIONS around the group's bbox center by the same factor as the elements (`_grpScaleBase` baseline set on selection) — "A + B" grows as one picture instead of letters growing into fixed gaps; exactly reversible; single-element behavior unchanged.

---

**Prior — July 10, 2026 (night)** — **PARAMETRIC CATCH shipped + Card Maker note↔card LINKS + paste-at-click**, work tip = this commit, deployed to the canonical trio. **(1) Parametric CATCH** (commit `8da5f16`, deployed `2c86332`): the Find pipeline runs on `savedCatchGames` — `_pfRefreshCatchBake` re-bakes on Library ▶; the player's Catch pool deals 3 instances per parametric pair via the shared `_paramChooseInstances`; each instance's problem slot is FROZEN and its answer slot FLOATING, so Catch's native round = the frozen problem ("3 + 2 ="), its true answer falling as the match, the other instances' answers as distractors; "Repeats OK" checkbox in the Catch game view; `_catchBuildCardSVG` now strips author chrome on EVERY face (label/note boxes + loose note text + `pm-param-boxes` — fixed note leaks on non-parametric bubbles too). **(2) Note↔card CONNECTION in the Card Maker** (user request): drag a NOTE (white data-info card) over a normal card → gold ATTACH glow (insertion line hidden); drop = the note lands right AFTER the card ("Note attached after A1" flash) — no left/right-half ambiguity. Attached notes show a **dashed-gold outline + a gold bridge** to their card (`_pmApplyNoteLinks`, re-applied by `_applyRoleBadges` after every rebuild; adjacency = the association the games read). The parallel stream layered an EXPLICIT link store on the same drop: **`cardNoteLinks_v1`** (`_cnl*`: loupe ＋-attach, detach with undo toast, glow/chain visuals) — both recorded on attach. **(3) Loupe paste-at-click** (user spec): paste centers the copied group's bbox on the LAST CLICKED point on the card (clamped fully on-card — the old "+5/+5 from source position, click ignored" rule made bottom-right groups land too low/right and off-edge); the pasted group arrives SELECTED as a whole (drag/arrows move it together; empty-space click "declicks"); repeat pastes cascade +4; no click yet → legacy nudge; click memory resets per card. Also: the loupe visibility-EYE edit guard, bake link-inference, and the level-driven board count all landed earlier today (see below).

---

**Prior — July 10, 2026** — **🔒 PLAYER-SIDE SECRECY PRINCIPLE decided + parametric FIND-board spec agreed** (docs-only; see `docs/MEMORY.md` §July 10 — READ IT before any parametric-play work). The user's standing concern: a player-hacker must not be able to steal a game's generating program — so the player side gets ONLY baked outputs (concrete instances / frozen faces); the `_mp*`/`_pm*` engine + the authored math stores never ship to `index.html`/`js/game.js` and never sync to player devices. The July 9 Catch plan's "phase 2 = live in-game generation" is REJECTED (phase 2 = bake-refresh in the Studio). Agreed Find-board spec (option 1): Studio bakes a digit-diverse instance pool (all instances, capped; answers computed) at game save / Studio launch, stored with the game; player deals from it — **no repeated answer within a board page (red column)**, **different problems across 2–3 consecutive plays** via per-game history, author-side "Repeats OK" override (mirrors the worksheet rule). **BUILT + DEPLOYED same day (later July 10):** `_pfBakeFindGame`/`_pfRefreshFindBake` (pm-studio, refreshes on Library ▶) bake `game.paramBake` {instances: k/a/p/s value maps, links}; index.html `_paramChooseInstances` deals **3 instances per problem card** (min 2 — never one domino; K slots per card, labels `·N`), answers distinct across the deal, `paramPlayHistory_v1` avoids the last 2–3 plays (relax 3→2→1), "Repeats OK" checkbox in the Find game view; problem+answer share an instance & take the ANSWER as match value → the double = problem over its true answer; the level (2/3/4 dominos) drives the board count (1 double + N−1 decoys, verified all levels). Play faces strip author chrome: `g.pm-param-boxes` + label boxes **including loose note text geometrically inside them** (`_stripCardLabelsMarkup`). Missing problem↔answer links are INFERRED at bake (same formula text, one hides / one shows the answer sym — manual answer cards work; explicit links win); the visibility EYE now runs the per-game edit-guard dialog like an art edit (was a silent write-through).

---

**Prior — July 9, 2026** — **MATH-CREATOR UX ROUND + EDIT GUARD + PARAMETRIC CATCH PHASE 1**, work tip = this commit (see git log), deployed to `claude/review-project-docs-JOOeh` + mirrors. All in `pm-studio-DrV.html` + 2 lines in `index.html`. **(1) Add-picker**: Sets/insert-from-set shows a MINIMAL bar (Copy + ✕ only; bar text explains "these cards are currently not in “game”"; empty-Copy → quiet reminder); Gam got a **line PICKER** (step 2: only lines not yet in the game, click-select, Copy/✕). **(2) Setup ergonomics**: info column aligns across lines (1–2-card lines share the after-two-cards position via measured `.mg-cardzone` min-width; 3+-card lines push individually); click any background = deselect all; selection clicks BLUR focused inputs (the "keyboard delete doesn't work" fix — Delete/↑↓ then act on the selection); **note cards drag with the mouse** (onto a line = attach, onto a note's left/right half = ordered insert, onto the background = own loose row; `_mgMoveNote`). **(3) Missing cards**: unresolvable game cards show an honest "⚠ card not found — remade or deleted" chip (was: garbled internal label); resolution gained a LABEL fallback; note selections snapshot their svgMarkup so remakes can't blank previews. **(4) EDIT GUARD** (user spec): saving a loupe edit to a game-used card asks per game — "You are changing the card in **line C** of **Math Problems: “M Try1”** game" (yellow emphasis) — UN-CHECKED games keep the old card: it's FROZEN into Safe Haven (art + params/formula/rel cloned to a new uid) and the game re-points (entries + roles/blanks/weights/zones keys remapped). `drawSave` → `_drawSaveCore` wrapper, `_svFindCardUsage`/`_svFreezeOldCard`/`_svRepointGame`. **(5) PARAMETRIC CATCH phase 1** (plan in `docs/HANDOVER_2026-07-07_MATH-GAMES.md` §PARAMETRIC CATCH): **＋ Answer card** (i-panel) drops a LINKED real card showing the answer symbol (math data copied, answer visible); link store **`cardAnswerLinks_v1`** {answerUid: problemUid}; answer cards ride their problem's line in the Math setup (green "answer" tag) and never self-generate; **`_pcBakePairs(game,n)`** bakes same-instance face pairs (weighted line draws; DISTINCT answers via retry-WITHIN-the-line — re-drawing the line on collision inverted the weights, fixed). REMAINING: "Play as Catch" button (transient catch game from pairs; read game.js pairing first). **(6) NOTES IN FIND/CATCH fixed** (user report: notes lost background+status in a Catch game): `buildCardFromMarkup` re-applies `data-info` (graph-paper look in game views), edit-preselect keeps `isNote`, `_gvBackfillNoteFlags` migration backfills legacy games on open, and **index.html's two play pools (Find dominos + Catch bubbles) skip `isNote` cards** — notes can never enter play (the note-BOX on faces was already stripped by `_stripCardLabelsMarkup`). index.html got ONLY those 2 filter lines — coordinate with the parallel session.

---

### Prior — July 8, 2026 (after midnight, Math-games stream) — **WORKSHEET INPUT MODES DEPLOYED**: work tip **`10b1d93`** → deploy merge **`dfe5153`** (+ both mirrors), banner **12:01 AM PDT**. The worksheet player has **two input modes** (Input select in the ws bar AND the game setup; `ws.input`, default mouse; switching applies to the current page without regenerating): **Mouse** = no keyboard on screen; **Touch** = a fixed **TWO-line keypad** (`1 2 3 4 5 | ⌫` / `6 7 8 9 0 | ◀ back · ⏎ enter`), answers `readOnly` (no tablet keyboard), active answer gold, tap any answer to select it, ⏎ = next EMPTY problem (all filled → Check), ◀ = previous. **Digits DRAG off the keypad onto ANY problem in any order**: gold chip follows the finger, hovered row highlights green, release types the digit there (appends on a filled box; miss = no-op). Three load-bearing details: the whole `.ws-prob` ROW is the drop target (the bare input was a near-miss trap); the keypad goes `pointer-events:none` during a drag so rows behind it stay reachable; and the drop target is resolved BEFORE the keypad is restored (restoring first swallowed drops over the pad — real bug). Fresh pages show ONLY the "Attempt N of M" pill (the "Fill in the answers…" text is gone, user request). Full detail: the **July 8 addendum** in `docs/HANDOVER_2026-07-07_MATH-GAMES.md`.

---

**Date**: July 8, 2026 (night session after the July 7 deploy) — **PER-RULE FIX PASS + CARD MAKER ROLE FLOW DEPLOYED**, deploy-branch merge **`4aa1a33`** (all 3 branch tips verified identical), work tip **`ca56f53`**. Four user-reported bugs fixed, one feature added, all in `pm-studio-DrV.html`: **(1) Par panels invisible** — `98c33d4` had moved the shared `.par-panel` styles (z-index/background/border) onto `#par-panel`, so f/Rel/▷/i opened BEHIND the card; shared styles restored to the class, `#par-panel` keeps only `width:452px`. **(2) Per-rule constructs ignored by the problem space** (`b0a6748`) — `_mpValueSet`/`_mpJointSpace`/`_mpValueCount` dispatched on legacy `p.mode==='constructed'`+`p.def` (empty for rule-targeted constructs) and enumerated the raw range (A=10a+b b∈{1..8} still produced 60/90); all three now route through `_mpGenerateAny` gated on `_mpUsesPerPart` — verified 82-value pool, zero forbidden endings, legacy params byte-identical. **(3) ＋ Info card / i-panel now itemize per-rule params** (`_pmPerPartInfo`: domain : construct + restricted digit sets + constraints per rule, − for Subtract, "(all)" for shared) — the July 7 "known not-done" is DONE; old Info cards are frozen snapshots, remake them. **(4) Select box** — a plain click on empty grid space exits Select mode (iPad!); card taps + mouse-band still keep it. **(5) NEW rail Role button** under + ⇤ W V: with a selection → the 'Role for N cards' picker directly; without → role-pending Select mode (`_cmRolePending`) where the NEXT card tap opens the Role box (never the bulk menu), Save applies + exits, Cancel keeps the flow; the Select box's own 'Role (N)…' stays. `ca56f53` also carries the PARALLEL session's Math-game setup line-selection work (click/marquee select, Delete/↑↓ on selection, drag-a-line with drop bar, Esc clears-then-closes) — committed together at the user's "commit all". Deploy was done in a TEMP WORKTREE (shared tree never left `work/cardmaker-rowcopy`). Open: user real-data pass on the Par fixes; Catch/Math note hooks + tickets consuming notes; user will decide later if the Select box's Role item can go. (The sync.js `savedMathGames` cache-buster is DONE — `local-wins-16`, in HEAD; the "still pending" note was stale — see the Math-Games section below.)

---

**Date**: July 7–8, 2026 (the PARALLEL stream of the same night) — **MATH PROBLEMS SAVED GAMES (Interactive Worksheets)**, same tips (work **`ca56f53`**, deploy **`4aa1a33`**). "Math Problems" is now a REAL saved game type: GM → + New Game → Math Problems → name + card selection (bar gains **Card only / Card + its notes / Just the notes**; notes = trailing `data-info` cards) → saved to **`savedMathGames`** (sync LOCAL-WINS + backups, `sync.js?v=local-wins-16`) → a Math-specific **SETUP screen `#mg-overlay`** (NO columns): per line a literal **Card-Set copy** of the card + its note cards (stored-data fallback `_mgCardDataFor` when the Card Maker DOM isn't loaded; internal names hidden; `data-info` graph-paper preserved), a **"Player sees" SAMPLE problem** (blank value half-transparent, ↻), **"Show a problem" eye chips** = per-GAME `hiddenOverrides` (card's eye = default, ↺), **Role + Weight** in named **Mix presets** (`probOptions`; weights RELATIVE — total shown, honest fractional %); **line management by GESTURE** (marquee/click select, drag-a-line with gold drop bar, Delete/Backspace, ↑/↓ nudge — capture-phase so the Card Maker never sees the keys; toolbar keeps only common-Weight + Clear). **▶ = worksheet FROM the game** (`wsOpenGeneratorForGame`): each problem slot draws its card independently by weight (largest-remainder was replaced — a 0.5% card must be RARE, not never; verified ≈4/100 pages), notes/faces never shown to the player; worksheet **list numbers = problem size** (lighter, 22px gap) and the author sets **Per column** (`ws.perCol`, numbering runs DOWN each column). Full detail + gotchas: **`docs/HANDOVER_2026-07-07_MATH-GAMES.md`** (read this first for Math-game work). NOTES MODEL (user-corrected twice): notes = the card's anchored label-box + the white `data-info` cards; author-only, ALWAYS; the player gets plain text problems. Next: math 📝 recording hooks (`_gnScopedGame` is find-only), Game-Preview/new-tab play for math, roles→notes prefill.

---

**Date**: July 7, 2026 — **PAR "RULES" REDESIGN + report fixes DEPLOYED**, deploy-branch merge **`1ac7ebb`** (Firebase run green), work tip **`263cd71`**. The Par editor now models a parameter as numbered **rules** (ranges/sets) each with Add/**Subtract**, its own construct + constraints, per-rule Check/Sample and paging Sample; the editor opens fully collapsed (✎ opens a rule's bordered box, ▴ folds it to a summary like "2: ÷{3,5,7}"); the parameter list draws each rule in a rectangle joined by +/− with restricted micros in YELLOW (hover = missing digits). Engine is gated on `_mpUsesPerPart` — legacy params generate identically (verified). Game-Notes reports: legend re-read at play start + `gameNoteLegends_v1` backup (the "wrong notes recorded" bug), roles = LOWER card only via the badges' stableId lookup, D2 visual dominoes, '+'-in-label made-parsing, faces frozen per session (`session.faces`) + guaranteed domino fallback. Card Maker: white label-card copies keep `data-info`; right-click "Reset size"; group-move no longer strays the emptied line's placeholder into the destination (empty lines persist now). **Read `docs/HANDOVER_2026-07-07_PAR-RULES.md` first** — full function map + the two-session tree rules (NEVER `git add -A`; the parallel session's Math-Games feature rode into `f38378e` and its sync.js `savedMathGames` local-wins entry still needs a cache-buster bump). Open: user real-data pass, ＋ Info card per-rule summary, Catch/Math note hooks + tickets.

---

**Date**: July 5, 2026 — **VISUAL notes reports + new-tab play + mini-game chips**, tip **`364ea79`**. The Library **▶ opens the Game Preview in a NEW TAB** (`bdfbc56`, other session) — the Notes report **rehomed to Studio tab FOCUS** (`_gnCheckFreshSession`: fresh session → document; `gameNotePreviewPing` without a session → explanatory toast; running session → silence + a live **"▶ playing now"** entry in 📄 Reports; stale-only orphan folding, 10-min heartbeat `lastFlushAt`). **REPORTS ARE VISUAL**: A1/A2/A3/A4/B1/B2/B3 rows draw the domino from its two **REAL card faces** — `made` ("E1+B3") recorded by game.js from the deck half-keys; `_gnCardFaces` resolves inline svgMarkup → own set store → ALL `customDrawnCards_*` (stableId → sharedArtId → label) → **variations** = original's face + their SVG transform; one unresolvable half renders as text INSIDE the domino frame. Timing notes = chronological **chips** (face + green/correct, red/wrong times); **A5 missed-per-role** resolves card ROLES from made-labels at view time (both halves count) → red count bars. C1–C4, D2, D3 still text. GP Setup: **mini-game CHIPS** (see/load/adjust/💾 update-or-create/✎ rename-only, Default protected), **Play left-aligns with the Type column**, name-box width clamps (2-pass layout convergence), subtitle "— change on Home page" link. User is actively testing legends (currently Notes=A5+B2+B3). Full detail: `docs/HANDOVER_2026-07-04_GAME-NOTES.md` + this section. **DEPLOYED**: everything through `364ea79` merged → `claude/review-project-docs-JOOeh`.

---

### Prior — July 4, 2026 — **GAME NOTES system + digit-aware sampling + loupe UX**, commit **`c5389c3`** in `pm-studio-DrV.html` (+ a 2-line hook in `js/game.js`). The **Game Notes box** (📝, Library games column) became a real pipeline: notes have stable ids, on/off **toggles**, **parameters** prompted in a movable dialog (values = editable gold chips), **All on / All off**, an **applies-per-game-type** filter (Find/Catch/Math chips; inapplicable notes greyed `n/a` in place), per-type **template plans** (`activeGameNotes_v2`). Each **Find game owns versioned note-set LEGENDS** — `gameNoteSets` on the saved game, versions named by signature (**"GameName with Notes=A1+A2+C3"**), 📝 button per game row, `+ version` / `🗑` / chip-select; the **active version records** during a preview (template = fallback). Recording: play iframe starts a session, `game.js` (`sunLevelWin`/`sunLevelWrongCard`) reports judged answers via `window._gnRecord` (inert otherwise; `game.js?v=game-notes-1`), events flush crash-safe, sessions land in `gameNoteSessions_v1` (last 10). **Reports**: evaluators for all 16 notes (clear-guess trigger, guessing-vs-chance, per-card/role %, timing/streaks, totals); the notes document **auto-opens when the preview closes**; **📄 Reports** = history. Also this session: **digit-aware diverse sampling** (each digit position a coordinate — Examples/preview/worksheet batches spread over decades AND ones digits), **no-repeat-previous-batch** rule (game-maker "Repeats" override + insufficient-space warning), T-tool **live typing** (type on card/label; auto-start with no click; Escape ends without closing loupe), **marquee from the backdrop**, **group arrow-nudge**, **label content individually selectable**, tickets/notes text edits. Full detail: **`docs/HANDOVER_2026-07-04_GAME-NOTES.md`** (read this first). Next steps agreed: Catch/Math recording hooks, then **tickets consuming notes** (transitions) — per-game legends + tickets = the minigame's behavioral spec. **DEPLOYED**: `work/cardmaker-rowcopy` → `claude/review-project-docs-JOOeh` (merge). NOTE: a **parallel session** works on this same branch/file (Tickets Box, Game Notes box shell, label anchor/lock, Card Maker help were its commits) — coordinate ships.

---

### Prior — June 29, 2026 — **counting overhaul + Info/Examples reference cards + Values-as-parts + math-symbol editing** in `pm-studio-DrV.html`. Values is now a dynamic list of **range/set PARTS** (union; replaces Range/Set/Both and "In set"); the **answer parameter** (`C` in `A+B=C`) is excluded from the count, its **range is optional** and **narrows** when set; the count is **always a number** (`≈ 3-sig × 10ᵏ` when large) and **cross-constraint counts are now EXACT/stable** (no wobble). Per-line **＋ Examples card** (number-form problems, space-box padded, random order) + **＋ Info card** (colour-coded param/relation summary); **transparency follows the visible/invisible eye** everywhere (not the formula answer). Twin-scan keys on a **parametric signature** (cards differing only in parameters aren't "identical"); clicking a `<tspan>` selects its parent `<text>` (coloured lines become movable); the **T-tool Math group is one shared set** (`_pmMathGlyphs`: no parens, + ∈ ∉, inlined `a/b` fraction) shared with the **Edit-text** dialog. Full detail: **`docs/HANDOVER_2026-06-29_COUNTING-INFOCARDS-SYMBOLS.md`** (read this first). Standing TODO: real eyeball pass on :8000 (param editor doesn't render in headless screenshots). **DEPLOYED**: `work/cardmaker-rowcopy` → `claude/review-project-docs-JOOeh` (merge) + both mirror branches fast-forwarded to the same tip. **Post-ship (June 29):** the row-label repair (`_maybeOfferRowRepair`) now **skips parametric cards** (a `data-param` card's first glyph is a parameter, not a row letter) so number/problem sets like "NumberProblem" no longer get a false "rows out of sync" prompt; a double-click on a `<tspan>` now edits its parent `<text>`. **An "alternative card format" experiment (the info as a white panel split onto the card / a tall divided card) was tried and FULLY DISCARDED** — `＋ Info card` and `＋ Examples card` stay **separate reference cards** added to the line, as before. (Why discarded: the card system fixes card height at 60 units and the loupe sizes the card as `60 × magnifier`, so a tall/split card can't be done without reworking the card-sizing core — see [[MEMORY]].)

### Prior — June 27–28, 2026 — big **parametric math-problems** session in `pm-studio-DrV.html`: a **"Math Problems" worksheet game type** (GM → New Game), **copy carries parameters/formula/relations**, **visible/invisible** parameters (GP0 eye → half-transparent on card), **"Print on card" as separate movable parts** (space-box sizes the box + layout, digit-width measured, whole line auto-fits), **multi-select loupe editing** (marquee + group move), **≠ / Int() / value-sets `{0,2,4}`** in the constraint language, **colour-coded parameters** (A magenta · B orange · C blue · D green · E red), a **colour-chip cross-parameter constraint builder** in Rel (numbered "Save relation N" boxes; `Int(A/10)≠Int(B/10)` enforced), a **range-aware place-value decomposition helper** in Construct (`D=100a+10b+c`, editable digit sets, tick-to-accept = Rule 1), and the **construction edited as numbered rule boxes**. Full detail: **`docs/HANDOVER_2026-06-28_PARAMETRIC-CONSTRAINTS.md`** (read this first). The real-loupe TODO is **DONE** (verified live on :8012), except the param editor panel didn't render in headless screenshots — worth an eyeball pass on :8000.
